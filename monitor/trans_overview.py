...
for item in response:
    ...
    if item['symbol'] != "EUR":
        ...
        for item in response:
            ...
            line_amount = float(item['amount'])
            if item['side'] == 'sell':
                sum = -1 * sum
                line_amount = -1 * line_amount
            sub_totaal += line_amount
            print(f"{count}, {dt}, {item['market']}, {item['side']}, {item['amount']}, {item['price']}, {item['fee']}, subtotaal: {sub_totaal}")
            count = count + 1
        print(f"TOTAL voor {item['market']}: aantal {count} = som {sum} totaal = {sub_totaal}")